...
def test_config_loader_loads_all_sections(tmp_path, monkeypatch):
    ...
    _write_json(
        char_dir / f"{char_name}.json", _character_payload(10000000, char_name)
    )
    (char_dir / "skills").mkdir(parents=True, exist_ok=True)
    (char_dir / "skills" / f"{char_name}_basic.py").write_text(
        "", encoding="utf-8"
    )
    (char_dir / "talent").mkdir(parents=True, exist_ok=True)
    (char_dir / "talent" / f"{char_name}_talent.py").write_text(
        "", encoding="utf-8"
    )
    (char_dir / "technique").mkdir(parents=True, exist_ok=True)
    (char_dir / "technique" / f"{char_name}_technique.py").write_text(
        "", encoding="utf-8"
    )
    (char_dir / "eidolons").mkdir(parents=True, exist_ok=True)
    (char_dir / "eidolons" / f"{char_name}_eidolon_1.py").write_text(
        "", encoding="utf-8"
    )
    (char_dir / "bonus_ability").mkdir(parents=True, exist_ok=True)
    (char_dir / "bonus_ability" / f"{char_name}_bonus_ability_1.py").write_text(
        "", encoding="utf-8"
    )
    ...
    _write_json(
        lc_dir / f"{lc_name}.json", _light_cone_payload(30000000, lc_name)
    )
    ...
    _write_json(
        relic_dir / "head.json",
        _relic_piece_payload(21000000, relic_set, "head"),
    )
...
def test_config_loader_returns_latest_version_when_unspecified(
    ...
    _write_json(
        char_v1 / f"{char_name}.json", _character_payload(10000001, char_name)
    )

    char_v2 = configs_root / "v2.0" / "characters" / char_name
    _write_json(
        char_v2 / f"{char_name}.json", _character_payload(20000001, char_name)
    )
